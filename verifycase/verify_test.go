...
func TestContractRevisionAfterDocDurable(t *testing.T) {
	root := t.TempDir()
	...
	if _, err := docSvc.Revise(contract.ID, "v2", now); err == nil {
		t.Fatal("revision must fail when the document body cannot be written")
	}
	...
	if current.Revision != 1 {
		t.Fatalf("revision advanced before content was durable: revision=%d", current.Revision)
	}
}
